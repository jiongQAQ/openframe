...
export function registerShotsHandlers() {
  ...
  ipcMain.handle('shots:getBySeries', (_event, seriesId: string) => {
    const raw = getRawDb()
    const rows = raw
      .prepare(
  ...
  ipcMain.handle('shots:insert', (_event, shot: ShotRow) => {
    const raw = getRawDb()
    raw
      .prepare(
  ...
  ipcMain.handle('shots:update', (_event, shot: ShotRow) => {
    const raw = getRawDb()
    raw
      .prepare(
  ...
  ipcMain.handle('shots:replaceBySeries', (_event, payload: { seriesId: string; shots: ShotRow[] }) => {
    const raw = getRawDb()
  ...
  ipcMain.handle('shots:delete', (_event, id: string) => {
    const raw = getRawDb()
